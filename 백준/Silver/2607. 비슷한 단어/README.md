...
### 출력 
 ...
 1. 기준 단어를 입력받고 알파벳별 사용횟수를 배열 word_alpha[26]에 저장한다
 2. 비교 단어를 입력받고 알파벳별 사용횟수를 배열에 str_alpha[26]에 저장한다
 3. 비교단어와 기준단어의 length가 2이상 차이나면 pass
 4. 알파벳별 사용횟수를 한 알파벳씩 각각 비교하고, 한 알파벳에서 차이가 2이상 나면 pass한다
 5.  또한 모든 알파벳에 대해 사용횟수가 3이상 차이나면 pass한다(알파벳 하나만 바꾼 단어인 경우 2회까지 차이날 수 있기 때문이다)
